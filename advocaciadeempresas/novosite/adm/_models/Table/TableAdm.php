...
$Read = new Read;
$Read->FullRead("SELECT id_user, usuario_user, email_user FROM n_usuarios WHERE id_user != :id AND user_level != :nivel AND status_user != 'R'", "id={$_SESSION['userlogin']['id_user']}&nivel=2");
$Rows = $Read->getRowCount();
$TotalRows = $Rows;

$Query = "SELECT id_user, usuario_user, email_user FROM n_usuarios WHERE id_user != :id AND user_level != :nivel AND status_user != 'R'";
