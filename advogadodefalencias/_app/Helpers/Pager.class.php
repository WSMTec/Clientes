...
//    private $Colunas;
    ...
    function __construct($Link, $First = null, $Last = null, $MaxLinks = null) {
        ...
        $this->MaxLinks = ( (int) $MaxLinks ? $MaxLinks : 1);
    }
...
//        $this->Colunas = (string) $Colunas;
    ...
    private function getSyntax() {
        ...
        if ($this->Rows > $this->Limit):
            ...
            $fist = ($this->Page == 1 ? 'class="btn-disabled"' : '');
            $last = ($this->Page == $Paginas ? 'class="btn-disabled"' : '');

            $this->Paginator .= "<li><a {$fist} title=\"{$this->First}\" href=\"{$this->Link}1\">{$this->First}</a></li>";

            if ($this->Page >= 5):
                $this->Paginator .= "<li><a title=\"{$this->First}\" href=\"{$this->Link}1\">1</a></li>";
                $this->Paginator .= "<li><a class='btn-disabled' title=\"{$this->First}\" href=\"{$this->Link}1\">...</a></li>";
            endif;
            ...
            $this->Paginator .= "<li><a {$last} title=\"{$this->Last}\" href=\"{$this->Link}{$Paginas}\">{$this->Last}</a></li>";
            $this->Paginator .= "</ul>";
        endif;
    }
