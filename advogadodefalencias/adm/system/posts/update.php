...
<main class="content">
    ...
    $Read = new Read;
    $Read->ExeRead("tb_posts", "WHERE post_id = :id", "id={$office}");
    ?>
    ...
    <div class="box content-box">
        <form method="post" class="form-posts" id="form-posts-update" action="posts-update">
            <div class="row-m">
                ...
                <select class="inpt-null" name="post_category" id="category">
                    <?php
                    $Read = new Read;
                    ...
                    foreach ($Read->getResult() as $d):
                        ...
                        <optgroup label="<?= $d['category_title']; ?>">
                            <?php
                            $Read->FullRead("SELECT * FROM tb_categories WHERE category_parent = :id", "id={$d['category_id']}");
                            foreach ($Read->getResult() as $v):
                                ?>
                                <option <?= ($post_category == $v['category_id'] ? 'selected' : ''); ?> value="<?= $d['category_name'] . '-' . $v['category_id']; ?>"><?= $v['category_title']; ?></option>
                            <?php endforeach; ?>
                        </optgroup>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="row-f row-video" style="<?= $post_type == 'video' ? "" : "display: none;"; ?>">
                <label>
                    <span class="lnr lnr-graduation-hat"></span>  Youtube (Apenas o código, ex: https://www.youtube.com/watch?v=tl69eScPCso, o código seria apenas <b>tl69eScPCso</b>)
                </label>
                <input class="inpt-null" name="post_video" type="text" value="<?= $post_video; ?>"/>
            </div>
            <div class="row-f">
                <label>
