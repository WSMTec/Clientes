...
class AdminPost {
    ...
    public function ExeCreate(array $Data) {
        ...
        if (empty($this->Data['post_title']) || empty($this->Data['post_content']) || empty($this->Data['post_category'])):
            $this->Error = ["Existem campos em branco. Favor preencha os campos de TÍTULO, CONTEÚDO e CATEGORIA para cadastrar!", "blue", "lnr lnr-warning", 4000];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
            ...
            if ($this->Data['post_cover']):
                ...
                $uplaod->Image($this->Data['post_cover'], $this->Data['post_name'], null, 'blog');
            endif;
    ...
    public function ExeUpdate($PostId, array $Data) {
        ...
        if (empty($this->Data['post_title']) || empty($this->Data['post_content']) || empty($this->Data['post_category'])):
            $this->Error = ["Existem campos em branco. Favor preencha os campos de TÍTULO, CONTEÚDO e CATEGORIA. ( Capa não precisa ser enviada! )", "blue", "lnr lnr-warning", 4000];
            $this->Result = false;
        else:
            $this->setData();
            $this->setName();
            ...
            if (is_array($this->Data['post_cover'])):
                ...
                $uploadCapa->Image($this->Data['post_cover'], $this->Data['post_name'], null, 'blog');
            endif;
...
//        if ($this->Data['post_video'] == 'null') {
//            $this->Data['post_type'] = 'post';
//        } else {
//            $this->Data['post_type'] = 'video';
//        }
        $this->Data['post_cover'] = $Cover;
        $this->Data['post_content'] = $Content;
        $cat = explode('-', $this->Data['post_category']);
        $this->Data['post_category'] = $cat[1];
    ...
    private function Create() {
        $this->Data['post_content'] = ($this->Data['post_content'] != 'null' ? $this->Data['post_content'] : null);
        $this->Data['post_video'] = ($this->Data['post_video'] != 'null' ? $this->Data['post_video'] : null);
        $News = $this->Data['post_newsletter'];
        if ($this->Data['post_newsletter'] == '1'):
            $this->Data['post_newsletter'] = null;
        endif;
        $cadastra = new Create;
...
//                var_dump($News);
                if (!$News->getResult()) {
                    $this->Error = ["O email não foi enviado!", "red", "lnr lnr-warning", 4000];
                    $this->Result = false;
                    return;
                }
            endif;
