...
//                    var_dump($Read->getRowCount());
        if ($Read->getRowCount()):
            ?>
            <span> <a style="color: rgb(215, 89, 74);" href="" class="like" id="unlike_<?= $post_id; ?>"><?= $post_like; ?> <img src="<?= INCLUDE_PATH; ?>/images/artigos/like.png" width=""/></a> </span>
            <?php
        else:
            ?>
            <span> <a href="" class="like" id="like_<?= $post_id; ?>"><?= $post_like; ?> <img src="<?= INCLUDE_PATH; ?>/images/artigos/like.png" width=""/></a> </span>
        <?php
        endif;
        ?>
    <?php else: ?>
...
<article class="comment_post content">
    ...
    if (!$comment->getResult()):
        ...
        foreach ($comment->getResult() as $data):
            ...
            <?php
            $Read->FullRead("SELECT * FROM tb_comment WHERE comment_visitor = :v AND comment_answer = :a", "v={$data['comment_visitor']}&a={$data['comment_id']}");
            if ($Read->getRowCount()):
                ?>
